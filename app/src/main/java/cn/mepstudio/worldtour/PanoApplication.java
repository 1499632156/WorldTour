...
public class PanoApplication extends Application {
    ...
    public static class MyGeneralListener implements MKGeneralListener {
        ...
        public void onGetPermissionState(int iError) {
            ...
            if (iError != 0) {
                // 授权Key错误：
         //       Toast.makeText(PanoApplication.getInstance().getApplicationContext(),"请在AndoridManifest.xml中输入正确的授权Key,并检查您的网络连接是否正常！error: " + iError, Toast.LENGTH_LONG).show();
            } else {
             //   Toast.makeText(PanoApplication.getInstance().getApplicationContext(), "key认证成功", Toast.LENGTH_LONG).show();
            }
        }
    }
}